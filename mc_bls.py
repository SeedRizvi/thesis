#!/usr/bin/env python3
"""
Monte Carlo BLS evaluation across manoeuvre configurations.

Runs each config with two modes:
  1. BLS-B: base BLS without Gaussian manoeuvre estimation
  2. BLS-G: Gaussian-augmented BLS estimating delta-v and t*

Per-seed results are saved to report_mc_bls.csv.
An aggregate summary is printed and saved to report_mc_bls_summary.csv.

Usage:
    python3 mc_bls.py
    python3 mc_bls.py --seeds 10
    python3 mc_bls.py --configs deltaRIC1 deltaI0.2
"""
...
# from Orbit_BLS_STM import SatelliteOrbitBLS_STM as SatelliteOrbitBLS
...
DEFAULT_OUTPUT = "report_mc_bls.csv"
DEFAULT_SUMMARY = "report_mc_bls_summary.csv"
...
def run_bls_seed(seed, truth_states, times, dt, ground_stations, params,
                 delta_v_ric, delta_v_eci, manoeuvre_state, t_star_true,
                 mode, config_tag):
    """
    Run one BLS solve and return a comprehensive metrics dict.

    mode: 'BLS-B' (no manoeuvre estimation) or 'BLS-G' (Gaussian estimation)
    """
    np.random.seed(seed)
    ...
    if mode == 'BLS-G' and delta_v_eci is not None:
    ...
    bls.run(max_iters=params['max_iterations'], verbose=False)
    runtime = time.perf_counter() - t0

    # Position / velocity errors
    errors = bls.states - truth_states
    ...
    if mode == 'BLS-G' and manoeuvres is not None and delta_v_ric is not None:
        ...
        result['t_star_error'] = float(bls.man_params[3] - t_star_true)
    else:
    ...
    if mode == 'BLS-G' and manoeuvres is not None and delta_v_ric is not None:
        ...
        if t_star_true is not None:
            ...
            plot_results['t_star_estimated'] = float(bls.man_params[3])
            plot_results['t_star_error'] = float(bls.man_params[3] - t_star_true)
...
def main():
    parser = argparse.ArgumentParser(
        description='Monte Carlo BLS evaluation across manoeuvre configs')
    ...
    print("=" * 80)
    print("Monte Carlo BLS Evaluation")
    ...
    if INITIAL_GUESSES:
        ...
        guesses_path = "report_mc_bls_initial_guesses.csv"
